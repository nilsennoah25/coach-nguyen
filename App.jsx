...
const sv = async d => {
  try {
    await setDoc(doc(_db, "coachapp", SK), { data: JSON.stringify(d) });
  } catch(e) {
...
const lv = async () => {
  try {
    const snap = await getDoc(doc(_db, "coachapp", SK));
...
const saveTmpl = async d => {
  try {
    await setDoc(doc(_db, "coachapp", TMPL_SK), { data: JSON.stringify(d) });
  } catch(e) { try { localStorage.setItem("coachapp_templates", JSON.stringify(d)); } catch(e2) {} }
};
const loadTmpl = async () => {
  try {
    const snap = await getDoc(doc(_db, "coachapp", TMPL_SK));
    if (snap.exists()) return JSON.parse(snap.data().data);
...
function AppSettingsTab({ brand, setBrand, darkMode }) {
  ...
  const save = async () => {
    ...
    try {
      const current = await lv() || {};
      await sv({ ...current, brand: newBrand });
    } catch(e) { console.error("Brand save error:", e); }
    setSaved(true);
