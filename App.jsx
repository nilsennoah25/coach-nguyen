                  ...
                  {setupTypes.map((mt) => {
                    ...
                    return (
                      <div key={mt.id} style={{ background:G.bg3, borderRadius:8, padding:"10px 12px" }}>
                        ...
                        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                          <button className="btn-icon" onClick={() => updateSetupRow(mt.id,"pct",(+mt.pct||0)-5)}>-</button>
                          <input type="range" min="0" max="70" step="5" value={mt.pct||0} onChange={e => updateSetupRow(mt.id,"pct",+e.target.value)} style={{ flex:1 }} />
                          <button className="btn-icon" onClick={() => updateSetupRow(mt.id,"pct",(+mt.pct||0)+5)}>+</button>
                        </div>
                      </div>
                    );
